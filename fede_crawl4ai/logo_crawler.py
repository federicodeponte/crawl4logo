...
class LogoCrawler:
    ...
    async def analyze_image_with_openai(
        ...
        if self.use_azure:
            return await self._analyze_image_with_azure(
                image_base64, image_url, page_url, html_element, page_html
            )
        else:
            return await self._analyze_image_with_regular_openai(
                image_base64, image_url, page_url, html_element, page_html
            )
    ...
    async def _analyze_image_with_azure(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a logo detection assistant. Analyze the image and determine if it's a logo. If it is, provide a confidence score (0-1) and description in this format: 'Confidence Score: X.XX\nDescription: ...'. If not, return 'null'.",
            },
            {
                "role": "user",
                "content": [
        ...
        data = {"messages": messages, "max_tokens": 300}
    ...
    async def _analyze_image_with_regular_openai(
        ...
        data = {"model": "gpt-4o-mini", "messages": messages, "max_tokens": 300}
    ...
    async def crawl_for_logos(
        self, start_url: str, max_pages: int = 10, output_file: Optional[str] = None
    ) -> List[LogoResult]:
    ...
    async def process_csv_batch(
        self, csv_file_path: str, output_dir: str = "results", confirm_header: bool = True
    ) -> Dict[str, List[LogoResult]]:
        """
                                ...
                                async with aiohttp.ClientSession() as session:
                                    async with session.get(result.url) as response:
                                        if response.status == 200:
                                            ...
                                            image_filename = (
                                                f"logo_{i+1}_{result.confidence:.2f}.png"
                                            )
                                            ...
                                            result_dict = {
                                                ...
                                                "background_removed_image_url": (
                                                    cloud_url if cloud_url else local_file_url
                                                ),
